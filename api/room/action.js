import { getRoom, saveRoom, recordGame } from "../../lib/redis.js";
...
function endQuestion(room, winner, pts) {
  ...
  if (room.doneBtns.length >= total) {
    ...
    // Fire-and-forget: record stats for both players (uses userId, not display name)
    if (room.hostId && room.guestId) {
      recordGame(
        room.hostId,  room.hostName,  s["1"],
        room.guestId, room.guestName, s["2"]
      ).catch(e => console.error("recordGame:", e.message));
    }
  }
}

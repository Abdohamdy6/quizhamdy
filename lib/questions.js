...
// ضفنا playedQuestions كـ Parameter عشان نستبعدهم
export function getCategories(playedQuestions = []) {
  const out = {};
  ...
  for (const folder of fs.readdirSync(QDIR).sort()) {
    ...
    for (const file of fs.readdirSync(fp).filter(f => f.endsWith(".json")).sort()) {
      try {
        ...
        // ✨ التعديل هنا: فلترة الأسئلة اللي اتلعبت قبل كده
        qs = qs.filter(q => !playedQuestions.includes(q.q));
...
// ضفنا playedQuestions هنا كمان عشان منختارش سؤال قديم
export function pickQuestions(rel, playedQuestions = []) {
  const d  = smartParse(path.join(QDIR, rel));
  let qs = d.questions || [];
  
  // ✨ التعديل هنا: استبعاد الأسئلة الملعوبة من السحب
  qs = qs.filter(q => !playedQuestions.includes(q.q));

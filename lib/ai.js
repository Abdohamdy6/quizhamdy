...
import { distance } from 'fastest-levenshtein'; // مكتبة بسيطة لحساب الفرق بين الكلمات

const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);

export async function judgeAnswer(question, correctAnswer, playerAnswer) {
    try {
        // --- ١. التنظيف والتحقق البرمجي (لتقليل استخدام الـ API) ---
        ...
        // أ) تطابق مباشر أو احتواء
        if (nPlayer === nCorrect || nCorrect.includes(nPlayer) || nPlayer.includes(nCorrect)) {
            if (nPlayer.length >= 2) return true;
        }

        // ب) تشابه إملائي بسيط (حرف واحد خطأ مثلاً)
        // إذا كانت المسافة بين الكلمتين حرف واحد فقط، نعتبرها صحيحة فوراً
        if (distance(nCorrect, nPlayer) <= 1) return true;


        // --- ٢. استخدام الموديل للحالات المعقدة (البرومت الخاص بك) ---
